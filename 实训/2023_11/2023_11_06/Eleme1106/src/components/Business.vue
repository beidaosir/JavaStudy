<template>
    <div>
        ...
        <input type="text" class="form-control" style="width: 200px;display: inline-block;" placeholder="请输入商家账号" v-model.trim="searchForm.businessAccount">
        <input type="text" class="form-control" style="width: 200px;display: inline-block;" placeholder="请输入商家名称" v-model.trim="searchForm.businessName">
        ...
        <table class="table table-striped" style="background-color: white">
            <thead>
                <tr>
                    ...
                    :checked="checkIds.length == currentPageList.length && checkIds.length != 0"></th>
                    <th scope="col">序号</th>
...
export default {
    ...
    methods: {
        search(){
            if(!this.searchForm.businessAccount && !this.searchForm.businessName){
                this.showList = this.businessList;
                return;
            }
            this.showList = this.businessList.filter((businessObj)=>{
               return businessObj.businessAccount.indexOf(this.searchForm.businessAccount)>=0 
                && businessObj.businessName.indexOf(this.searchForm.businessName)>=0
            })
        ...
        add() {
            ...
            //添加的时间 时间戳
            businessObj.datetime = Date.now();
            //设置默认密码
            businessObj.password = '111111';
            //设置角色
            businessObj.role = 'business';

            this.businessList.push(businessObj);//将要添加的对象放入集合
