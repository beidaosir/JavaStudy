...
public class FoodServiceImpl implements FoodService {
    ...
    @Transactional
    @Override
    public boolean updateExplain(Food food) {
        if (food==null){
            return false;
        }
        if (food.getFoodId()==null || food.getFoodExplain()==null || food.getFoodExplain().equals("")){
            return false;
        }
        return foodMapper.updateExplain(food)>0;
    }

    @Transactional(readOnly = true)
    @Override
    public String getExplain(Integer foodId) {
        if (foodId==null){
            return null;
        }
        return foodMapper.getExplain(foodId);
    }
}
